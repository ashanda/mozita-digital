...
final class ThemesFlat_Addon_For_Elementor_Bixos {
    ...
    public function i18n() {
        load_plugin_textdomain( 'bixos', false, basename( dirname( __FILE__ ) ) . '/languages' );
    }
    ...
    public function tf_admin_notice_missing_main_plugin() {
        ...
        $message = sprintf(
            esc_html__( '"%1$s" requires "%2$s" to be installed and activated.', 'bixos' ),
            '<strong>' . esc_html__( 'Themesflat Addons For Elementor', 'bixos' ) . '</strong>',
            '<strong>' . esc_html__( 'Elementor', 'bixos' ) . '</strong>'
        );
    ...
    public function admin_notice_minimum_elementor_version() {
        if ( isset( $_GET['activate'] ) ) unset( $_GET['activate'] );
        $message = sprintf(
            esc_html__( '"%1$s" requires "%2$s" version %3$s or greater.', 'bixos' ),
            '<strong>' . esc_html__( 'Themesflat Addons For Elementor', 'bixos' ) . '</strong>',
            '<strong>' . esc_html__( 'Elementor', 'bixos' ) . '</strong>',
    ...
    public function admin_notice_minimum_php_version() {
        ...
        $message = sprintf(
            esc_html__( '"%1$s" requires "%2$s" version %3$s or greater.', 'bixos' ),
            '<strong>' . esc_html__( 'Themesflat Addons For Elementor', 'bixos' ) . '</strong>',
            '<strong>' . esc_html__( 'PHP', 'bixos' ) . '</strong>',
    ...
    public function init_widgets() {
        ...
        require_once( __DIR__ . '/widgets/widget-testimonial-carousel-2.php' );
        \Elementor\Plugin::instance()->widgets_manager->register_widget_type( new \TFTestimonialCarousel2_Widget() );
        ...
        require_once( __DIR__ . '/widgets/widget-portfolios.php' );
        ...
        \Elementor\Plugin::instance()->widgets_manager->register_widget_type( new \TFHeading_Widget() );
    ...
    public function widget_styles() {
        ...
        wp_register_style( 'tf-services', plugins_url( '/assets/css/posts/tf-services.css', __FILE__ ) );
        wp_register_style( 'tf-portfolio', plugins_url( '/assets/css/posts/tf-portfolio.css', __FILE__ ) );
        ...
        wp_register_style( 'tf-heading', plugins_url( '/assets/css/heading/tf-heading.css', __FILE__ ) );
    ...
    public function admin_scripts() {
        ...
        $tfhf_localize_vars = array(
            ...
            'search'        => esc_html__( 'Search pages / post / categories', 'bixos' ),
    ...
    ========================================= */
        public function tf_header_footer_post_type() {
            $labels = array(
                'name'                  => esc_html__( 'TF Header - Footer Template', 'bixos' ),
                'singular_name'         => esc_html__( 'TF Header - Footer Template', 'bixos' ),
                'rewrite'               => array( 'slug' => esc_html__( 'TF Header - Footer Template' ) ),
                'menu_name'             => esc_html__( 'TF Header - Footer Template', 'bixos' ),
                'add_new'               => esc_html__( 'Add New', 'bixos' ),
                'add_new_item'          => esc_html__( 'Add New Template', 'bixos' ),
                'new_item'              => esc_html__( 'New Template Item', 'bixos' ),
                'edit_item'             => esc_html__( 'Edit Template Item', 'bixos' ),
                'view_item'             => esc_html__( 'View Template', 'bixos' ),
                'all_items'             => esc_html__( 'All Template', 'bixos' ),
                'search_items'          => esc_html__( 'Search Template', 'bixos' ),
                'not_found'             => esc_html__( 'No Template Items Found', 'bixos' ),
                'not_found_in_trash'    => esc_html__( 'No Template Items Found In Trash', 'bixos' ),
                'parent_item_colon'     => esc_html__( 'Parent Template:', 'bixos' ),
                'not_found'             => esc_html__( 'No Template found', 'bixos' ),
                'not_found_in_trash'    => esc_html__( 'No Template found in Trash', 'bixos' )
        ...
        public function tf_header_footer_register_metabox() {
            add_meta_box(
                'tfhf-meta-box',
                esc_html__( 'TF Header Or Footer Options', 'bixos' ), 
                ...
                <tbody>
                    <tr class="tfhf-options-row type-of-template">
                        <td class="tfhf-options-row-heading">
                            <label for="tfhf_template_type"><?php esc_html_e( 'Type of Template', 'bixos' ); ?></label>
                        </td>
                        <td class="tfhf-options-row-content">
                            <select name="tfhf_template_type" id="tfhf_template_type">
                                <option value="" <?php selected( $template_type, '' ); ?>><?php esc_html_e( 'Select Option', 'bixos' ); ?></option>
                                <option value="type_header" <?php selected( $template_type, 'type_header' ); ?>><?php esc_html_e( 'Header', 'bixos' ); ?></option>
                                <option value="type_footer" <?php selected( $template_type, 'type_footer' ); ?>><?php esc_html_e( 'Footer', 'bixos' ); ?></option>
                            </select>
                        </td>
                    </tr>
        ...
        public function tf_header_footer_metabox_rule() {  
            ...
            <tr class="tfhf-target-rules-row tfhf-options-row">
                <td class="tfhf-target-rules-row-heading tfhf-options-row-heading">
                    <label><?php esc_html_e( 'Display On', 'bixos' ); ?></label>
                </td>
                <td class="tfhf-target-rules-row-content tfhf-options-row-content">
                    ...
                    self::target_rule_settings_field(
                        ...
                        [
                            'title'          => esc_html__( 'Display Rules', 'bixos' ),
                            'value'          => '[{"type":"basic-global","specific":null}]',
                            'tags'           => 'site,enable,target,pages',
                            'rule_type'      => 'display',
                            'add_rule_label' => esc_html__( 'Add Display Rule Group', 'bixos' ),
                        ],
                        $include_locations
                    );
                    ?>
            ...
            <tr class="tfhf-target-rules-row tfhf-options-row">
                <td class="tfhf-target-rules-row-heading tfhf-options-row-heading">
                    <label><?php esc_html_e( 'Do Not Display On', 'bixos' ); ?></label>
                </td>
                <td class="tfhf-target-rules-row-content tfhf-options-row-content">
                    <?php
                    self::target_rule_settings_field(
                        ...
                        [
                            'title'          => esc_html__( 'Exclude On', 'bixos' ),
                            'value'          => '[]',
                            'tags'           => 'site,enable,target,pages',
                            'add_rule_label' => esc_html__( 'Add Exclusion Rule Group', 'bixos' ),
                            'rule_type'      => 'exclude',
                        ],
                        $exclude_locations
                    );
        ...
        public static function get_template_id( $type ) {
            ...
            $tfhf_templates = ThemesFlat_Addon_For_Elementor_Bixos::instance()->get_posts_by_conditions( 'tf_header_footer', $option );
    ...
    ======================================== */
        ...
        public static function get_location_selections() {
            ...
            $special_pages = array(
                'special-404'    => esc_html__( '404 Page', 'bixos' ),
                'special-search' => esc_html__( 'Search Page', 'bixos' ),
                'special-blog'   => esc_html__( 'Blog / Posts Page', 'bixos' ),
                'special-front'  => esc_html__( 'Front Page', 'bixos' ),
                'special-date'   => esc_html__( 'Date Archive', 'bixos' ),
                'special-author' => esc_html__( 'Author Archive', 'bixos' ),
            );

            if ( class_exists( 'WooCommerce' ) ) {
                $special_pages['special-woo-shop'] = esc_html__( 'WooCommerce Shop Page', 'bixos' );
            }

            $selection_options = array(
                'basic'         => array(
                    'label' => esc_html__( 'Basic', 'bixos' ),
                    'value' => array(
                        'basic-global'    => esc_html__( 'Entire Website', 'bixos' ),
                        'basic-singulars' => esc_html__( 'All Singulars', 'bixos' ),
                        'basic-archives'  => esc_html__( 'All Archives', 'bixos' ),
                    ),
                ),

                'special-pages' => array(
                    'label' => esc_html__( 'Special Pages', 'bixos' ),
                    'value' => $special_pages,
                ),
            );
            ...
            $selection_options['specific-target'] = array(
                'label' => esc_html__( 'Specific Target', 'bixos' ),
                'value' => array(
                    'specifics' => esc_html__( 'Specific Pages / Posts / Taxonomies, etc.', 'bixos' ),
                ),
            );
        ...
        public static function target_rule_settings_field( $name, $settings, $value ) {
            ...
            $add_rule_label = isset( $settings['add_rule_label'] ) ? $settings['add_rule_label'] : esc_html__( 'Add Rule', 'bixos' );
            $saved_values   = $value;
            $output         = '';
            ...
            $output .= '<option value="">' . esc_html__( 'Select', 'bixos' ) . '</option>';
        ...
        public static function get_post_target_rule_options( $post_type, $taxonomy ) {
            ...
            $all_posts                          = sprintf( esc_html__( 'All %s', 'bixos' ), $post_label );
            $post_option[ $post_name . '|all' ] = $all_posts;

            if ( 'pages' != $post_key ) {

                $all_archive                                = sprintf( esc_html__( 'All %s Archive', 'bixos' ), $post_label );
            ...
            if ( in_array( $post_type->name, $taxonomy->object_type ) ) {
                ...
                $tax_archive = sprintf( esc_html__( 'All %s Archive', 'bixos' ), $tax_label );
        ...
        public static function generate_target_rule_selector( $type, $selection_options, $input_name, $saved_values, $add_rule_label ) {
            ...
            if ( 'display' == $type ) {
                ...
                $output .= '<a href="#" class="button">' . esc_html__( 'Add Exclusion Rule Group', 'bixos' ) . '</a>';
                $output .= '</div>';
            }
    ...
    ======================================== */
        public function tf_add_section_parallax( \Elementor\Element_Section $section ) {
            $section->add_control(
                ...
                [
                    'label' => esc_html__( 'TF Parallax', 'bixos' ),
            ...
            $section->add_control(
                'tf_section_parallax',
                [
                    'label' => esc_html__( 'Parallax', 'bixos' ),
            ...
            $section->add_control(
                ...
                [
                    ...
                    'description' => esc_html__( 'This is analogous to the background-position css property. Specify coordinates as top, bottom, right, left, center, or pixel values (e.g. -10px 0px). The parallax image will be positioned as close to these values as possible while still covering the target element.', 'bixos' ),
                    'default' => '',
                    'options' => [
                        ''  => esc_html__( 'Default', 'bixos' ),
                        'center center'  => esc_html__( 'Center Center', 'bixos' ),
                        'center left'  => esc_html__( 'Center Left', 'bixos' ),
                        'center right'  => esc_html__( 'Center Right', 'bixos' ),
                        'top center'  => esc_html__( 'Top Center', 'bixos' ),
                        'top left'  => esc_html__( 'Top Left', 'bixos' ),
                        'top right'  => esc_html__( 'Top Right', 'bixos' ),
                        'bottom center'  => esc_html__( 'Bottom Center', 'bixos' ),
                        'bottom left'  => esc_html__( 'Bottom Left', 'bixos' ),
                        'bottom right'  => esc_html__( 'Bottom Right', 'bixos' ),
                        'custom'  => esc_html__( 'Custom', 'bixos' ),
                    ],
                    'condition' => [ 
            ...
            $section->add_control(
                ...
                [
                    'label' => esc_html__( 'X Position', 'bixos' ),
            ...
            $section->add_control(
                ...
                [
                    'label' => esc_html__( 'Y Position', 'bixos' ),
                    'type' => \Elementor\Controls_Manager::SLIDER,
                    'size_units' => [ 'px' ],
                    'range' => [
                        'px' => [
            ...
            $section->add_control(
                'tf_speed_parallax',
                [
                    'label' => esc_html__( 'Speed', 'bixos' ),
                    'type' => \Elementor\Controls_Manager::SLIDER,
                    'description' => esc_html__( 'The speed at which the parallax effect runs. 0 means the image will appear fixed in place, and 1 the image will flow at the same speed as the page content.', 'bixos' ),
                    'size_units' => [ 'px' ],
                    'range' => [
                        'px' => [
                            'min' => 0,
            ...
            $section->add_control(
                'tf_bleed_parallax',
                [
                    'label' => esc_html__( 'Bleed', 'bixos' ),
                    'type' => \Elementor\Controls_Manager::NUMBER,
                    'description' => esc_html__( 'You can optionally set the parallax mirror element to extend a few pixels above and below the mirrored element. This can hide slow or stuttering scroll events in certain browsers.', 'bixos' ),
                    'min' => 0,
                    'condition' => [ 
                        'tf_section_parallax' => 'yes',
                    ],
...
ThemesFlat_Addon_For_Elementor_Bixos::instance();