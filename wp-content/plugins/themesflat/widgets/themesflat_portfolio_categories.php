...
class Themesflat_Portfolio_Categories extends WP_Widget {
     ...
     * Constructor
     *
     * @return Themesflat_Portfolio_Categories
     */
    function __construct() {
        ...
        parent::__construct(
            'widget_categories_portfolio',
            esc_html__( 'Themesflat - Portfolio Categories', 'bixos' ),
            array(
                'classname'   => 'widget_categories_portfolio',
                'description' => esc_html__( 'Portfolio Categories.', 'bixos' )
            )
        );
    }
    ...
    function widget( $args, $instance ) {
        ...
        extract( $instance );
        extract( $args );
        $classes[] = 'widget widget_categories themesflat_widget_portfolio_categories';
            ...
            <ul>
                <?php if ( $custom_ids != '') {
                    ...
                    foreach ($categories_ids as  $id) {
                        if (get_term_by('id',$id,'portfolios_category') == true) {
                            $term = (get_term_by('id',$id,'portfolios_category'));
    ...
    function form( $instance ) {
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'custom_ids' ) ); ?>"><?php esc_html_e( 'Custom Categories By Ids- EX: 1,2,3', 'bixos' ); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'child_of' ) ); ?>"><?php esc_html_e( 'Child Of:', 'bixos' ); ?></label>
        ...
        <p>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'show_count' ) ); ?>"><?php esc_html_e( 'Show count?', 'bixos' ); ?></label>
        </p>
    <?php
    }
}

add_action( 'widgets_init', 'themesflat_register_portfolio_categories' );
 ...
 * @since 1.0
 */
function themesflat_register_portfolio_categories() {
