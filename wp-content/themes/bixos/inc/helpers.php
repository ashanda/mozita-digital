...
function themesflat_shortcode_default_id(){
    return array(
                ...
                'group' => esc_html__( 'Design Options', 'bixos' ),
...
function themesflat_predefined_header_styles() {
    ...
    if ( empty( $styles ) ) {
        $styles = apply_filters( 'themesflat/header_styles', array(
            'header-v1' => esc_html__( 'Classic', 'bixos' ),
            'header-v2' => esc_html__( 'Header style 02', 'bixos' ),
            'header-v4' => esc_html__( 'Modern', 'bixos' ),
        ) );
    }
...
function themesflat_menu_fallback() {
    ...
    <li>
    <a class="menu-fallback" href="' . esc_url(admin_url('nav-menus.php')) . '">' . esc_html__( 'Create a Menu', 'bixos' ) . '</a></li></ul>';
}
...
if ( version_compare( $GLOBALS['wp_version'], '4.1', '<' ) ) :
    ...
    function themesflat_wp_title( $title, $sep ) {
        ...
        if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
            $title .= " $sep " . sprintf( esc_html__( 'Page %s', 'bixos' ), max( $paged, $page ) );
        }
            ...
            <ul class="unstyled">
                <li class="current">                    
                    <a href="?lang=en" class="lang_sel_sel">
                        <span class="languages-before-icon fa fa-globe"></span><?php echo esc_html__("English",'bixos');?><i class="fa fa-chevron-down" aria-hidden="true"></i>
                    </a>
                    <ul class="unstyled-child">
                       <li class="icl-en">
                            <a href="?lang=en" class="lang_sel_sel">
                             <?php echo esc_html__("English",'bixos');?>
                            </a>
                       </li>
                       <li class="icl-fr fr">
                            <a href="?lang=fr" class="lang_sel_other">
                             <?php echo esc_html__("French",'bixos');?>
                            </a>
                       </li>
                       <li class="icl-ge ge">
                            <a href="?lang=it" class="lang_sel_other">
                                <?php echo esc_html__("German",'bixos');?>
                            </a>
                       </li>
                    </ul>
                </li>
...
function themesflat_custom_search_form( $form ) {
    ...
    <label>
        <span class="screen-reader-text">' . esc_html__( 'Search for:' , 'bixos' ) . '</span>
        <input type="search" value="' . get_search_query() . '" name="s" class="s" placeholder="' . esc_html__( "Enter Your Keyword...", "bixos" ) . '"/>
    </label>
...
function themesflat_social_single() {
    if( themesflat_get_opt('show_social_share') == 1 ):
        ...
        ?>
        <div class="social-share-article"><h6><?php echo esc_html__( 'Share :', 'bixos' ); ?></h6>        
...
function themesflat_get_page_titles() {
    ...
    if ( ! is_archive() ) {       
        ...
        elseif ( is_page() ) {
            ...
            $title = esc_html__( '404', 'bixos' );
        } elseif ( is_search() ) {
            $title = sprintf( esc_html__( 'Search results for &#8220;%s&#8221;', 'bixos' ), get_search_query() );
        } else {
            $title = get_post_meta( get_the_ID(), 'custom_title', true );
            if ( ! $title ) {
                $title = get_the_title();
        ...
        if ( is_author() ) {
            the_post();
            $title = sprintf( esc_html__( 'Author: %s', 'bixos' ), get_the_author() );
            rewind_posts();
        } elseif ( is_day() ) {
            $title = sprintf( esc_html__( 'Daily: %s', 'bixos' ), get_the_date() );
        } elseif ( is_month() ) {
            $title = sprintf( esc_html__( 'Monthly: %s', 'bixos' ), get_the_date( 'F Y' ) );
        } elseif ( is_year() ) {
            $title = sprintf( esc_html__( 'Yearly: %s', 'bixos' ), get_the_date( 'Y' ) );
        } elseif ( is_search() ) {
            $title = sprintf( esc_html__( 'Search results for &#8220;%s&#8221;', 'bixos' ), get_search_query() );
            ...
            $title = esc_html__('Book Appointment','bixos');           
